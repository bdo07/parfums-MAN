...
const ProductList = () => {
  const tousproduct = useSelector((state) => state.tousproduct);
  ...
  <div className="image-item left">
    ...
    <button className="af" onClick={handleShowMen}>parfum Men</button>
  </div>
  

  <div className="image-item right">
    <img src="/public/backg/rt.jpg" alt="women" />
    <button className="af" onClick={handleShowWomen}>parfum Femme</button>
...
>
  {womenn.slice(0, 4).map((product) => (  // تعديل هنا
    <div
      key={product.id}
      style={{
    ...
    >
      ...
      <button className="buy-button" onClick={() => navigate(`/parfum/${product.id}`)}>
        Acheter
      </button>
    </div>
...
>
  {men.slice(0, 4).map((product) => (  // تعديل هنا
    <div
      key={product.id}
      style={{
        border: "1px solid #ccc",
